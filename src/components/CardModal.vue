  ...
  <v-layout row justify-center>
            ...
            <v-layout wrap>
                ...
                >
                  <template v-slot:activator="{ on }">
                    <div class="board-header-btn1 board-header-btn-name">
                      <button class="board-header-btn-text" v-on="on">Due Date</button>
                    </div>
                  </template>
                  <div>
                    <div style="margin:10px;">
                      ...
                      <v-flex>
                        <v-text-field label="Date" v-model="date" outline></v-text-field>
                      </v-flex>
                      <v-flex>
                        <v-select :items="['0-17', '18-29']" label="Set Reminder" outline></v-select>
                      </v-flex>
                      <v-btn color="blue" flat @click="dueDateDialog = false">Save</v-btn>
                      <v-btn color="red" flat @click="dueDateDialog = false">Remove</v-btn>
                    </div>
                  </div>
                </v-menu>
              </div>
              ...
              <div class="text-xs-center">
                ...
                >
                  <template v-slot:activator="{ on }">
                    <div class="board-header-btn1 board-header-btn-name">
                      <button class="board-header-btn-text" v-on="on">Members</button>
                    </div>
                  </template>
                  <div>
                    <div style="margin:10px;">
                      <v-flex>
                        <v-text-field label="Search Members" outline></v-text-field>
                      </v-flex>
                    </div>
                  </div>
                </v-menu>
              ...
              <div class="text-xs-center">
                ...
                >
                  <template v-slot:activator="{ on }">
                    <div class="board-header-btn1 board-header-btn-name">
                      <button class="board-header-btn-text" v-on="on">CheckList</button>
                    </div>
                  </template>
                  <div>
                    <div style="margin:10px;">
                      ...
                      <v-btn color="blue" flat @click="checkListDialog = false">Add</v-btn>
                    </div>
                  </div>
                </v-menu>
              </div>
              ...
              <v-flex xs12 sm12 md12>
                ...
                <p
                  v-if="!showDescription"
                >{{ selectedCard.description || "Add more detailed description"}}</p>
                <v-btn
                  v-if="showDescription"
                  class="vbutton"
                  color="blue"
              ...
              <v-flex xs12 sm12 md12>
                <div v-for="cmnt in selectedCardsComments" :key="cmnt.id">
                  <p style="font-weight:bold; margin-left:10px;margin-bottom:8px;">Mitul Malani</p>
                  <span
                  ...
                  <div v-if="showComment==cmnt.id">
                    <v-textarea outline rows="1" auto-grow v-model="cmnt.comment"></v-textarea>
                    <v-btn
                      class="vbutton"
                      color="blue"
                      flat
                    ...
                    >Save</v-btn>
                    <v-btn class="vbutton" color="red" flat @click="showComment = false">Cancel</v-btn>
                  </div>
              ...
              <v-flex xs12 sm12 md12>
                ...
                <v-textarea outline rows="1" auto-grow v-model="comment"></v-textarea>
                <v-btn
                  class="vbutton"
                  color="blue"
                  flat
              ...
              <v-flex xs12 sm12 md12>
                ...
                <v-progress-linear v-model="checkListCompleted"></v-progress-linear>
                ...
                <v-text-field v-if="showAddItem" label="Title" v-model="addItem" outline></v-text-field>
                <v-btn
                  v-if="showAddItem"
                  class="vbutton"
                  color="blue"
                ...
                <v-btn
                  ...
                  @click="showAddItem = false"
                >Cancel</v-btn>
                <v-btn v-if="!showAddItem" color="blue" flat @click="showAddItem = true">Add an item</v-btn>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
    ...
    <div class="text-xs-center">
      <v-dialog v-model="deleteCommentDialog" width="300">
        <v-card>
          <v-card-title style="font-size:15px;">Are you sure to delete Comment?</v-card-title>
          <v-divider></v-divider>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="red" flat @click="deleteComment">Delete</v-btn>
            <v-btn color="primary" flat @click="deleteCommentDialog = false">Cancel</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </div>
